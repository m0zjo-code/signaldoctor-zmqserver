...
for i in range(0, no_sigs):
    ...
    spec_size = 256
    feature_dict = sdl.generate_features(1, noise_vec, spec_size, plot_features = False, config = config, roll=False)
...
for i in range(0, no_sigs):
    ...
    feature_dict['iq_data'] = noise_vec
    feature_dict['local_fs'] = 1
    sdl.save_IQ_buffer(feature_dict, output_folder = output_folder_carrier,  config = config, roll=False)
